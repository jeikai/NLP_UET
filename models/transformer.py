...
lao_tokenizer = laonlp.Tokenizer()


def vi_tokenize(sentence: str) -> [str]:
    tokens, b = pyvi.ViTokenizer.spacy_tokenize(sentence)
    return tokens


def lo_tokenize(sentence: str) -> [str]:
    return lao_tokenizer.word_tokenize(sentence)


def preprocess(array):
    """
    Xóa các kí hiệu đặc biệt của hệ thống khỏi một array của String.

    Args:
        array: Một array của String.

    Returns:
        Một array của String mới, không có các kí hiệu đặc biệt của hệ thống.
    """

    # Lấy danh sách các kí hiệu đặc biệt của hệ thống.
    system_symbols = ["\n", "\u200b", "\u200c", "\u200e"]

    # Tạo một array mới để lưu trữ kết quả.
    new_array = []

    # Duyệt qua từng String trong array.
    for string in array:
        # Kiểm tra xem String có chứa bất kỳ kí hiệu đặc biệt nào không.
        if string in system_symbols:
            # Nếu có, thì xoá String khỏi array.
            continue
        else:
            # Nếu không, thì thêm String vào array mới.
            new_array.append(string)

    for new_string in new_array:
        for string in system_symbols:
...
class Transformer(nn.Module):
    ...
    def __init__(self, mode=None, model_dir=None, config=None):
        ...
        self.SRC = data.Field(lower=opt.get("lowercase", const.DEFAULT_LOWERCASE),
                              tokenize=lo_tokenize, preprocessing=preprocess)
        self.TRG = data.Field(lower=opt.get("lowercase", const.DEFAULT_LOWERCASE), eos_token='<eos>',
                              tokenize=vi_tokenize, preprocessing=preprocess)
    ...
    def run_train(self, model_dir=None, config=None):
        ...
        for epoch in range(checkpoint_idx, opt['epochs']):
            ...
            for i, batch in enumerate(self.train_iter):
                # raise ValueError(f"Src: {batch.src}\nTrg: {batch.trg}")
    ...
    def run_infer(self, features_file, predictions_file, src_lang=None, trg_lang=None, config=None, batch_size=None):
        ...
        with io.open(features_file, "r", encoding="utf-8") as read_file:
            inputs = [process_line(l) for l in read_file.readlines()]
